...
def create_result_template(target: Target) -> dict:
    ...
    """
    result = {
        "data": {"tcp": {"status": "tcp", "result": {"response": {"request": {}}}}}
    }
    if target.ssl_check:
        tls_log = {
            "handshake_log": {
...
def create_error_template(
    target: Target, error_str: str, description: str = ""
) -> Dict:
    """
...
def make_document_from_response(
    buffer: bytes, target: Target, addition_dict: Dict = None, protocol: str = ""
) -> dict:
    """
    ...
    # endregion
    try:
        result["data"]["tcp"]["result"]["response"]["body_raw"] = b64encode(
            buffer
        ).decode("utf-8")
