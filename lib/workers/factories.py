...
def create_target_tcp_protocol(
    ip_str: str, target_config: TargetConfig
) -> Iterator[Target]:
    """
...
def get_generator(target_config: TargetConfig) -> Optional[PayloadGenerator]:
    ...
    payloads_generator = load_python_generator_payloads_from_file(
        path_to_module, func_name
    )
    return payloads_generator


def create_targets_tcp_protocol(
    ip_str: str, settings: TargetConfig
