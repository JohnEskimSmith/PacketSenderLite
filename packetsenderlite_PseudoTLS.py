...
from lib.workers import get_async_writer, create_io_reader, TargetReader, TaskProducer, Executor, OutputPrinter, \
    TargetWorker
from lib.util import parse_settings, parse_args, single_read, access_dot_path
from lib.core import Stats, create_error_template, make_document_from_response, Target, TargetConfig
...
class TargetWorkerPseudoTLS(TargetWorker):
...
async def main():
    ...
    async with aiofiles_open(config.output_file, mode=config.write_mode) as file_with_results:
        ...
        target_worker = TargetWorkerPseudoTLS(statistics,
                                     task_semaphore,
                                     queue_prints,
